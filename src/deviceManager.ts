...
async function wake(deviceId: string, interaction: Interaction | CommandInteraction, requiredPermissions: devicePermission): Promise<{result: ActionResult, device?: string, mac?: string}> {
    ...
    try {
        ...
        return {result: ActionResult.Success, device: device.name, mac: device.network.macAddress};
    } catch (error) {
        return {result: ActionResult.ActionFailed, device: device.name, mac: device.network.macAddress};
    }
